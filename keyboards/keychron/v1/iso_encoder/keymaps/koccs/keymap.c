...
static bool gaming_effect_enabled = false;
static uint8_t previous_matrix_mode = 0;

bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  switch (keycode) {
    case GAME_TOG:
      if (record->event.pressed) {
        if(!gaming_effect_enabled) {
            previous_matrix_mode = rgb_matrix_get_mode();
            rgb_matrix_mode(RGB_MATRIX_CUSTOM_gaming_effect);
            gaming_effect_enabled = true;
        } else {
            rgb_matrix_mode(previous_matrix_mode);
            gaming_effect_enabled = false;
        }
      }
...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    ...
    if (host_keyboard_led_state().scroll_lock) {
        RGB_MATRIX_INDICATOR_SET_COLOR(29, 255, 0, 0);
        RGB_MATRIX_INDICATOR_SET_COLOR(43, 255, 0, 0);
        RGB_MATRIX_INDICATOR_SET_COLOR(58, 255, 0, 0);
    } else {
        if (!rgb_matrix_get_flags()) {
            RGB_MATRIX_INDICATOR_SET_COLOR(29, 0, 0, 0);
            RGB_MATRIX_INDICATOR_SET_COLOR(43, 0, 0, 0);
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [MAC_FN] = LAYOUT_iso_83(
         ...
         _______,  _______,  _______,                                GAME_TOG,                                _______,  _______,    _______,  _______,  _______,  _______),
